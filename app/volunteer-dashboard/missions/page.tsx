    ...
    <div className="space-y-10 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500">Discover where your skills are needed most right now.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
         {loading ? (
            [1,2,3,4].map(i => <div key={i} className="h-48 bg-white rounded-3xl border border-slate-200 animate-pulse"></div>)
         ) : openTasks.length === 0 ? (
            <div className="lg:col-span-2 bg-white p-20 rounded-[2rem] text-center border border-slate-200 shadow-sm">
                ...
                <p className="text-slate-500">Try a different search term or check back shortly for new requests.</p>
            </div>
         ) : (
            openTasks.map((task) => (
               <motion.div 
               ...
               >
                  <div>
                    <div className="flex justify-between items-start mb-6">
                       ...
                       <div className="text-right">
                          <p className="text-[10px] font-semibold text-slate-500 mb-1">Posted</p>
                          <p className="text-xs font-semibold text-slate-800">{new Date(task.createdAt).toLocaleDateString()}</p>
                       </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-10">
                    ...
                    <button className="py-4 rounded-2xl bg-primary hover:bg-primary/90 text-white font-black transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg flex items-center justify-center gap-2">
                      Accept <ChevronRight className="w-5 h-5" />
                    </button>
                  </div>
               </motion.div>
            ))
